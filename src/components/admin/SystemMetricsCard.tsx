...
const SystemMetricsCard: React.FC<{ metrics: SystemMetrics | null }> = ({ metrics }) => {
  ...
  return (
    <div className="p-3 bg-white rounded shadow space-y-2">
      ...
      <MetricRow label="CPU %" value={metrics.cpu_usage_percent ? `${metrics.cpu_usage_percent.toFixed(1)}%` : '-'} />
      <MetricRow label="Memory %" value={metrics.memory_usage_percent ? `${metrics.memory_usage_percent.toFixed(1)}%` : '-'} />
      <MetricRow label="Disk %" value={metrics.disk_usage_percent ? `${metrics.disk_usage_percent.toFixed(1)}%` : '-'} />
      <MetricRow label="Load (1m)" value={metrics.load_average?.one?.toFixed(2) ?? '-'} />
